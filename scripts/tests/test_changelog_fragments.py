...
class ChangelogFragmentTests(unittest.TestCase):
    ...
    def test_entry_must_be_one_or_more_bullets(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            path.write_text("  Indented, but no bullet yet.\n", encoding="utf-8")
            with self.assertRaisesRegex(
                fragments.FragmentError, "Markdown bullets"
            ):
                fragments.read_fragment_entries(path)
    ...
    def test_entry_may_wrap_onto_indented_continuation_lines(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            path = Path(temporary_directory) / "change.entry.md"

            path.write_text(
                "- A long bullet that wraps onto a second\n"
                "  physical line, like existing changelog entries do.\n"
                "- Another bullet.\n",
                encoding="utf-8",
            )
