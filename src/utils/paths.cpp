...
namespace utils
{
    ...
    /**
     * @brief Resolve an XDG-style directory using an environment variable with
     * a fallback under $HOME.
     *
     * This function attempts to read an environment variable specified by @p
     * env_name. If that variable is set and non-empty, its value is returned as
     * a std::filesystem::path wrapped in PathResult. If the environment
     * variable is missing or empty, the function falls back to using the user's
     * home directory (via unix_home) and appends @p fallback_suffix to it (e.g.
     * ".config" or
     * ".local/share").
     *
     * Notes:
     ...
     * @return PathResult Either a resolved std::filesystem::path on success or
     * an unexpected EnvError propagated from unix_home.
     */
    PathResult unix_xdg(
     ...
     * @return std::filesystem::path The resolved folder path, or an empty path
     * on error.
     */
     ...
     * base directory.
     * @return PathErrorResult The resolved per-user application directory path
     * or an PathError on failure.
     */
     ...
     * directories.
     *
     * If @p path is empty this function returns PathError::Empty. When @p path
     * is non-empty, std::filesystem::create_directories is invoked with a
     * non-throwing overload that accepts a std::error_code; on failure this
     * function returns PathError::FailedCreate and on success it returns @p
     * path.
     *
    ...
    PathErrorResult ensure_dir(const std::filesystem::path& path)
    {
        ...
        // catches cases where the path exists but is not a directory, or where
        // a race or permission issue prevented the directory from being usable.
     ...
     * directory via user_dir(DirKind::Config, app_name) and then calls
     ...
     * directory via user_dir(DirKind::Data, app_name) and then calls ensure_dir
     * to create it on disk if it does not yet exist.
     *
     * @param app_name Name of the application subdirectory.
     * @return PathErrorResult The ensured data directory path, or an PathError
     * on failure.
     */
