...
namespace orm
{
    class FieldView
    {
       private:
        ...
        bool             _is_auto_increment_primary_key{false};
       ...
       public:
        ...
        [[nodiscard]] bool             is_primary_key() const noexcept;
        [[nodiscard]] bool             is_auto_increment() const noexcept;
        [[nodiscard]] bool is_auto_increment_primary_key() const noexcept;
        ...
        static FieldView from(FieldType& field)
        {
            FieldView view{};
            view._mutable_field_address = static_cast<void*>(&field);
            view._const_field_address   = static_cast<void const*>(&field);

            view._column_name       = FieldType::name.view();
            view._is_primary_key    = FieldType::is_primary_key;
            view._is_auto_increment = FieldType::is_auto_increment;
            view._is_auto_increment_primary_key =
                FieldType::is_auto_increment_primary_key;
                                     ...
                                     int            index)
            {
                auto const* typed_field =
                    static_cast<FieldType const*>(field_address);
            ...
            {
                auto* typed_field = static_cast<FieldType*>(field_address);
                typed_field->read_from(statement, col);
            };

            view._ddl_function = []() -> std::string
            { return FieldType::ddl(); };

            return view;
        }

        template <typename FieldType>
        static FieldView from(FieldType const& field)
        {
            FieldView view{};
