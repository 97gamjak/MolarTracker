...
class ChangelogTests(unittest.TestCase):
    def test_release_notes_require_a_bullet(self):
        ...
        self.assertTrue(
            CHANGELOG.has_release_notes(
                ["", "### Bug Fixes", "", "- Fix a crash on startup."]
            )
        )
    ...
    def test_stamp_keeps_empty_next_release_and_history(self):
        ...
        stamped = CHANGELOG.stamp_release(
            head, body, tail, "v1.1.0", "97gamjak/Molartracker"
        )
    ...
    def test_stamp_preserves_wrapped_continuation_lines(self):
        lines = [
            "# Changelog",
            "",
            "## Next Release",
            ...
            "### Bug Fixes",
            "",
            "- Fix a crash on startup that only reproduced when the",
        ...
        self.assertIn(
            "- Fix a crash on startup that only reproduced when the", stamped
        )
        self.assertIn("  settings file was missing.", stamped)
    ...
    def test_release_routes_fragments_and_preserves_unreleased_entries(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            user_fragment = (
                changes_dir / "user" / "enhancement.startup.md"
            )
            ...
            self.assertIn("- Fix a crash on startup.", user_text)
            self.assertIn(
                "- Add a startup splash screen.",
                user_text,
            )
            ...
            self.assertNotIn("splash screen", dev_text)
