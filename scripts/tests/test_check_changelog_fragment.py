...
class ChangelogFragmentCheckTests(unittest.TestCase):
    ...
    def test_accepts_fragment_with_wrapped_bullet(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            path = root / "changes" / "user" / "enhancement.wrapped.md"
            path.parent.mkdir(parents=True)
            path.write_text(
                "- A bullet that wraps onto a second, indented\n"
                "  physical line.\n",
                encoding="utf-8",
            )

            errors = CHECK.validate_pr_changes(
                [("A", "changes/user/enhancement.wrapped.md")], root
            )
